...
let mongoData = (d) => {
    ...
    for (let t = 0; t < 50000; t++) {
        fiftyKAgents.push(`"agent":{"agentname":"${faker.name.findName()}","premier":"${faker.random.boolean()}","company":"${faker.company.companyName()}","reviews":"${faker.random.number(100)}","recentSales":"${faker.random.number(100)}","phone":"${faker.phone.phoneNumberFormat(1)}","imgurl":"${faker.image.avatar()}"}`)
    }
    
    let writeStream = fs.createWriteStream('./database/seed/datafiles/mongoHomes.JSON', { flags : 'w' });
    writeStream.write('[')
    for (let k = 1; k <= a; k++) {
        let lines = ''
        for (let i = 1; i <= 1000; i++) {
            ...
            let randAgent = fiftyKAgents[Math.floor(Math.random()*fiftyKAgents.length)]
            let messages = ``;
            if(rand > 0.5) {
