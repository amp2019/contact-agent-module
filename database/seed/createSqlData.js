...
let homesFile = (d) => {
    ...
    let c = d / 1000;
    let counter = 0
    let agentCeiling = 10000 - 2
    let writeStream = fs.createWriteStream('database/seed/datafiles/homesfiletenk.csv', { flags : 'w' });
    writeStream.write(headLine)
    for (let k = 1; k <= c; k++) {
        let lines = ''
        for (let i = 1; i <= 1000; i++) {
...
let messagesFile = (d) => {
    ...
    let b = d / 1000;
    let counter = 0
    let writeStream = fs.createWriteStream('database/seed/datafiles/messagesfiletenk.csv', { flags : 'w' });
    writeStream.write(headLine)
