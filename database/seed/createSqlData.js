...
let messagesFile = (d) => {
    ...
    for (let k = 1; k <= b; k++) {
        ...
        for (let i = 1; i <= 1000; i++) {
            ...
            lines += (1+faker.random.number(d-2)) + "," + faker.name.findName() + "," + faker.phone.phoneNumberFormat(1) + "," + faker.internet.email() + "," + hipsumArray.slice(hipRand,hipRand+10).join(' ') + "," + dateTime + '\n'
        }
        counter += 1000;
        writeStream.write(lines)
    }
